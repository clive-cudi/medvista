  ...
  User.findOne({ id: id, usertype: "patient" })
    .then((user) => {
      if (user) {
        ...
        Diagnosis.find({ diagnosisId: { $in: diagnoses_ids } })
          .then((diagnoses) => {
...
const getMedicalHistoryByID = (req: Request, res: Response) => {
  ...
  User.findOne({ id: userId, usertype: "patient" })
    .then((user) => {
      if (user) {
        // check if the diagnosis id is in the patient's diagnoses
        const diagnosis_id = user.patient.diagnoses.filter(
          (diagnosis_id) => diagnosis_id === id
        );

        if (diagnosis_id.length > 0) {
          Diagnosis.findOne({ diagnosisId: id })
            .then((diagnosis) => {
              return res.status(200).json({
                success: true,
                message: "Medical history found",
...
const createMedicalHistory = (req: Request, res: Response) => {
  ...
  const { diagnosisId, doctor, patient, date, symptoms, diagnosis, treatment } =
    req.body;
  ...
  User.findOne({ id: userId, usertype: "patient" })
    .then((user) => {
      if (user) {
        ...
        const diagnosisExists = user.patient.diagnoses.find(
  ...
  User.findOne({ id: userId, usertype: "patient" })
    .then((user) => {
      if (user) {
        ...
        Diagnosis.findOneAndUpdate(
          { diagnosisId: id },
          { isPendingDelete: true, isApproved: false }
        ).then((diagnosis) => {
          if (diagnosis) {
            // update the doctor's pending approvals
            User.findOneAndUpdate(
              { id: diagnosis.doctor, usertype: "doctor" },
              { $push: { "doctor.pendingDeletions": id } }
            )
              .then((doctor) => {
                return res.status(200).json({
                  success: true,
                  message:
                    "Medical history flagged for deletion and pending approval",
                  usertoken: {
                    user: user,
                    token: usertoken.token,
                  },
  ...
  User.findOne({ id: userId, usertype: "patient" })
    .then((user) => {
      if (user) {
        ...
        const doctors = [
          ...activeDoctors.map((a_doc) => ({ id: a_doc, status: "active" })),
          ...inActiveDoctors.map((i_doc) => ({
            id: i_doc,
            status: "inactive",
          })),
          ...archivedDoctors.map((ar_doc) => ({
            id: ar_doc,
            status: "archived",
          })),
        ];
          ...
          .then((found_doctors) => {
            ...
            for (let i = 0; i < found_doctors.length; i++) {
              const targetDoctor = doctors.find(
                (dctr) => dctr.id === found_doctors[i].id
              );

              if (targetDoctor) {
                doctorStatusConstruct[targetDoctor.status] = [
                  ...doctorStatusConstruct[targetDoctor.status],
                  targetDoctor.id,
                ];
              }
        ...
        User.find({
          ...
          .then((found_doctors) => {
            ...
            for (let i = 0; i < found_doctors.length; i++) {
              ...
              const { doctor, password, ...doc } = {
                ...found_doctors[i]._doc,
                doctor: {},
                password: "",
                speciality: found_doctors[i].doctor.speciality,
              };
...
const approveMedicalGlimpseRequest = (req: Request, res: Response) => {
  ...
  if (!doctorId) {
    return res.status(403).json({
      success: false,
      message: "No target doctor id provided",
      usertoken: {
        user: usertoken,
        token: usertoken.token,
      },
...
const searchPatient = (req: Request, res: Response) => {
  ...
  User.find({ usertype: "patient", name: { $regex: name, $options: "i" } })
    .then((patients) => {
      if (patients) {
        return res.status(200).json({
          success: true,
        ...
        newAppointment
          ...
          .then((new_appointment) => {
            ...
            if (doctor.doctor.inActivePatients.includes(userID)) {
              ...
              doctor.doctor.inActivePatients = [
                ...doctor.doctor.inActivePatients,
              ].filter((target_patient_id) => target_patient_id !== userID);
            }
            if (doctor.doctor.archivedPatients.includes(userID)) {
              // remove from inactive patients
              doctor.doctor.archivedPatients = [
                ...doctor.doctor.archivedPatients,
              ].filter((target_patient_id) => target_patient_id !== userID);
            }
            doctor
...
const updateAppointment = (req: Request, res: Response) => {
  ...
  Appointment.findOneAndUpdate(
    { appointmentId: appointmentId, [usertype]: userID },
    {
...
const removeAppointment = (req: Request, res: Response) => {
  ...
  Appointment.findOneAndDelete({ appointmentId: appointmentId })
    .then((deleted_appointment_result) => {
      // update appointment refs in doctor and patient
      if (deleted_appointment_result) {
        User.findOneAndUpdate(
          { id: deleted_appointment_result.patient, usertype: "patient" },
          {
            $pullAll: {
              "patient.appointments": [
                deleted_appointment_result.appointmentId,
              ],
          ...
          .then((updated_patient) => {
            User.findOneAndUpdate(
              { id: deleted_appointment_result.doctor, usertype: "doctor" },
              {
                $pullAll: {
                  "doctor.appointments": [
                    deleted_appointment_result.appointmentId,
    ...
    .then((user) => {
      if (user) {
        ...
        if (appointments.includes(appointment_id)) {
          ...
          Appointment.findOne({ appointmentId: appointment_id })
            .then((found_appointment) => {
          ...
          return res.status(403).json({
            message:
              "Not Authorized to view appointment or the appointment wasn't found",
            usertoken: {
              user: usertoken,
              token: usertoken.token,
            },
