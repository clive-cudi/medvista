...
export const RegularBtn = ({label, variant = "contained", withIcon, isLoading, className, children, ...other}: RegularBtn_Props): JSX.Element => {
    return (
        <button className={`${styles.regular_btn} ${styles[`regular_btn_${variant}`]} ${isLoading?.status == true ? styles.isLoading : ""} ${className ?? ""}`} disabled={isLoading?.status} {...other}>
