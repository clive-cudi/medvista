...
export const useDoctorStore = create<DoctorStore>()((set) => {
  return {
    ...
    add(doctor, doctor_state) {
      return set((state) => {
        return {
          doctors: {
            ...state.doctors,
            [doctor_state]: getUniqueListBy(
              [...state.doctors[doctor_state], doctor],
              "id"
            ),
          },
        };
      ...
      return set((state) => {
        return {
          doctors: {
            ...state.doctors,
            [doctor_state]: getUniqueListBy(
              [...state.doctors[doctor_state], ...doctors],
              "id"
            ),
          },
        };
    ...
    remove(doctor_id) {
      return set((state) => {
        return {
          doctors: Object.assign(
            {},
            ...Object.entries(state.doctors).map(
              (doctor_entry: [string, Doctor[]]) => ({
                key: doctor_entry[0],
                value: doctor_entry[1].filter((doc_) => doc_.id !== doctor_id),
              })
            )
          ),
        };
        ...
        return {
          doctors: Object.assign(
            ...
            ...Object.entries(state.doctors).map(
              (doctor_entry: [string, Doctor[]]) => ({
                key: doctor_entry[0],
                value: doctor_entry[1].filter(
                  (doc_) => !doctor_ids.includes(doc_.id)
                ),
              })
            )
          ),
