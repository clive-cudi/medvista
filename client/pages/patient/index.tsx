  ...
  const switchBtns: { btnComponent: JSX.Element | ReactNode }[] = [
    ...
    },
    {
      btnComponent: (
        <SideNavBtn
          variant={"primary"}
          isActive={currentTab.currentTab === "myDoctors"}
          onClick={() => {
            switchTab("myDoctors");
          }}
          withIcon={{ status: true, icon: <FaUserMd fontSize={20} /> }}
        >
          {navMin ? null : "My Doctors"}
        </SideNavBtn>
      ),
    },
    {
      btnComponent: (
        <SideNavBtn
          variant={"primary"}
          isActive={currentTab.currentTab === "myRecords"}
          onClick={() => {
            switchTab("myRecords");
          }}
          withIcon={{
            status: true,
            icon: <BsFillFileEarmarkTextFill fontSize={20} />,
          }}
        >
          {navMin ? null : "My Records"}
        </SideNavBtn>
      ),
    },
    {
